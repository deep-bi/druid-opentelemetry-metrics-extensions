...
public class OpenTelemetryMetricsProtobufReader implements InputEntityReader {
    ...
    public CloseableIterator<InputRow> read() {
        Supplier<Iterator<InputRow>> supplier =
                Suppliers.memoize(() -> readAsList().iterator());
    ...
    List<InputRow> readAsList() {
        ...
        try {
            ...
            // Explicitly move the position assuming that all the remaining bytes have been consumed because the
            // protobuf
    ...
    private List<InputRow> parseMetricsData(final MetricsData metricsData) {
        return metricsData.getResourceMetricsList().stream()
                .flatMap(resourceMetrics -> {
                    Map<String, Object> resourceAttributes = resourceMetrics.getResource().getAttributesList().stream()
                            .collect(
                                    HashMap::new,
                                    (m, kv) -> {
    ...
    private List<InputRow> parseMetric(Metric metric, Map<String, Object> resourceAttributes) {
        ...
        switch (metric.getDataCase()) {
            case SUM: {
                ...
                metric.getSum().getDataPointsList().forEach(dataPoint -> {
            ...
            case GAUGE: {
                ...
                metric.getGauge().getDataPointsList().forEach(dataPoint -> {
                    if (hasRecordedValue(dataPoint)) {
                        inputRows.add(parseNumberDataPoint(dataPoint, resourceAttributes, metricName));
                    }
                });
    ...
    private static boolean hasRecordedValue(NumberDataPoint d) {
        return (d.getFlags() & DataPointFlags.DATA_POINT_FLAGS_NO_RECORDED_VALUE_MASK.getNumber()) == 0;
    }

    private InputRow parseNumberDataPoint(
            NumberDataPoint dataPoint, Map<String, Object> resourceAttributes, String metricName) {

        int capacity = resourceAttributes.size() + dataPoint.getAttributesCount() + 2; // metric name + value columns
