...
public class OpenTelemetryBenchmark {
    ...
    private static final InputRowSchema ROW_SCHEMA = new InputRowSchema(
            null,
    ...
    private static final OpenTelemetryMetricsProtobufInputFormat INPUT_FORMAT =
            new OpenTelemetryMetricsProtobufInputFormat("name", "value", "", "resource.");
    ...
    private ByteBuffer createMetricBuffer() {
        ...
        for (int i = 0; i < resourceMetricCount; i++) {
            ...
            for (int j = 0; j < instrumentationScopeCount; j++) {
                ScopeMetrics.Builder scopeMetricsBuilder = resourceMetricsBuilder.addScopeMetricsBuilder();
                ...
                for (int k = 0; k < metricsCount; k++) {
                    ...
                    for (int l = 0; l < dataPointCount; l++) {
                        NumberDataPoint.Builder dataPointBuilder =
                                metricBuilder.getSumBuilder().addDataPointsBuilder();
    ...
    public void measureSerde(Blackhole blackhole) throws IOException {
        for (CloseableIterator<InputRow> it = INPUT_FORMAT
                        .createReader(ROW_SCHEMA, new ByteEntity(BUFFER), null)
                        .read();
                it.hasNext(); ) {
